...
describe("AstOutlineService", () => {
  ...
  function mockError(stdout: string, stderr: string, code: number): void {
    mockedExecFile.mockImplementation(
      (_cmd: string, _args: string[], _opts: object, cb: (err: Error | null, result?: { stdout: string; stderr: string }) => void) => {
  ...
  describe("constructor", () => {
    it("uses default binary path", () => {
      ...
      expect(mockedExecFile).toHaveBeenCalledWith("ast-outline", expect.any(Array), expect.any(Object), expect.any(Function));
    });
    ...
    it("accepts custom binary path", () => {
      ...
      expect(mockedExecFile).toHaveBeenCalledWith("/usr/local/bin/ast-outline", expect.any(Array), expect.any(Object), expect.any(Function));
    });
    ...
    it("accepts custom timeout", () => {
      ...
      customService.outline(["test.ts"]);
      expect(mockedExecFile).toHaveBeenCalledWith("ast-outline", expect.any(Array), expect.objectContaining({ timeout: 5000 }), expect.any(Function));
    });
  });
  ...
  describe("outline", () => {
    it("calls with correct base arguments", async () => {
      ...
      expect(mockedExecFile).toHaveBeenCalledWith("ast-outline", ["outline", "src/main.py"], expect.any(Object), expect.any(Function));
    ...
    it("passes --json flag", async () => {
      ...
      expect(mockedExecFile).toHaveBeenCalledWith("ast-outline", ["outline", "src/", "--json"], expect.any(Object), expect.any(Function));
    });

    it("passes --imports flag", async () => {
      mockSuccess("import os");
      await service.outline(["test.py"], { imports: true });
      expect(mockedExecFile).toHaveBeenCalledWith("ast-outline", ["outline", "test.py", "--imports"], expect.any(Object), expect.any(Function));
    });
    ...
    it("handles multiple paths", async () => {
      ...
      expect(mockedExecFile).toHaveBeenCalledWith("ast-outline", ["outline", "src/a.ts", "src/b.ts", "lib/"], expect.any(Object), expect.any(Function));
    });
  });
  ...
  describe("digest", () => {
    it("calls with correct arguments", async () => {
      ...
      expect(mockedExecFile).toHaveBeenCalledWith("ast-outline", ["digest", "src/"], expect.any(Object), expect.any(Function));
    ...
    it("passes --json flag", async () => {
      ...
      expect(mockedExecFile).toHaveBeenCalledWith("ast-outline", ["digest", "src/", "--json"], expect.any(Object), expect.any(Function));
    });
  });
  ...
  describe("show", () => {
    it("calls with file and single symbol", async () => {
      ...
      expect(mockedExecFile).toHaveBeenCalledWith("ast-outline", ["show", "Player.py", "TakeDamage"], expect.any(Object), expect.any(Function));
    ...
    it("calls with multiple symbols", async () => {
      ...
      expect(mockedExecFile).toHaveBeenCalledWith("ast-outline", ["show", "Player.py", "TakeDamage", "Heal", "Die"], expect.any(Object), expect.any(Function));
    });

    it("passes --json flag", async () => {
      mockSuccess("{}");
      await service.show("test.ts", ["foo"], { json: true });
      expect(mockedExecFile).toHaveBeenCalledWith("ast-outline", ["show", "test.ts", "foo", "--json"], expect.any(Object), expect.any(Function));
    });

    it("passes --signature flag", async () => {
      mockSuccess("def foo(x: int) -> str");
      await service.show("test.py", ["foo"], { signature: true });
      expect(mockedExecFile).toHaveBeenCalledWith("ast-outline", ["show", "test.py", "foo", "--signature"], expect.any(Object), expect.any(Function));
    });
  });
  ...
  describe("grep", () => {
    it("calls with pattern and paths", async () => {
      ...
      expect(mockedExecFile).toHaveBeenCalledWith("ast-outline", ["grep", "TakeDamage", "src/"], expect.any(Object), expect.any(Function));
      expect(result.stdout).toContain("TakeDamage");
    });
    ...
    it("passes --json flag", async () => {
      ...
      expect(mockedExecFile).toHaveBeenCalledWith("ast-outline", ["grep", "foo", "src/", "--json"], expect.any(Object), expect.any(Function));
    });

    it("passes --kind flag", async () => {
      mockSuccess("output");
      await service.grep("foo", ["src/"], { kind: "def" });
      expect(mockedExecFile).toHaveBeenCalledWith("ast-outline", ["grep", "foo", "src/", "--kind", "def"], expect.any(Object), expect.any(Function));
    });

    it("passes -w flag for word match", async () => {
      mockSuccess("output");
      await service.grep("foo", ["src/"], { wordMatch: true });
      expect(mockedExecFile).toHaveBeenCalledWith("ast-outline", ["grep", "foo", "src/", "-w"], expect.any(Object), expect.any(Function));
    });

    it("passes -i flag for case insensitive", async () => {
      mockSuccess("output");
      await service.grep("foo", ["src/"], { caseInsensitive: true });
      expect(mockedExecFile).toHaveBeenCalledWith("ast-outline", ["grep", "foo", "src/", "-i"], expect.any(Object), expect.any(Function));
    });

    it("passes -l flag for files only", async () => {
      mockSuccess("src/main.py");
      await service.grep("foo", ["src/"], { filesOnly: true });
      expect(mockedExecFile).toHaveBeenCalledWith("ast-outline", ["grep", "foo", "src/", "-l"], expect.any(Object), expect.any(Function));
    });

    it("passes -c flag for count", async () => {
      mockSuccess("src/main.py:5");
      await service.grep("foo", ["src/"], { count: true });
      expect(mockedExecFile).toHaveBeenCalledWith("ast-outline", ["grep", "foo", "src/", "-c"], expect.any(Object), expect.any(Function));
    });

    it("passes -m flag for max count", async () => {
      mockSuccess("output");
      await service.grep("foo", ["src/"], { maxCount: 5 });
      expect(mockedExecFile).toHaveBeenCalledWith("ast-outline", ["grep", "foo", "src/", "-m", "5"], expect.any(Object), expect.any(Function));
    });
  ...
  describe("isAvailable", () => {
    it("returns true when binary exists", async () => {
      ...
      expect(mockedExecFile).toHaveBeenCalledWith("ast-outline", ["--version"], expect.any(Object), expect.any(Function));
    });
